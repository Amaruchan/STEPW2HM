...
class Cache:
  ...
  def access_page(self, url, contents):
      new_access = {url:contents}
      if new_access not in self.current_cache:
          self.current_cache = [new_access] + self.current_cache[:-1]
      else:
          self.current_cache.remove(new_access)
          self.current_cache = [new_access] + self.current_cache
      pass
  ...
  def get_pages(self):
      ans = []
      for dict in self.current_cache:
          if dict == -1: break
          ans.append(dict.keys()[0])
      return ans
